/*------------------------------------------------------------
 * 文件：i2c.c
 * 作用：软件模拟 I2C（bit-bang）底层实现。
 * 框架：提供 Start/Stop/Write/Read/Ack 原语，上层设备驱动在此基础上拼装协议帧。
 *-----------------------------------------------------------*/
#include <REGX52.H>
#include <intrins.h>
#include "i2c.h"

/*
 * I2C 时序微延时：每个 _nop_() 约 1us（@11.0592MHz/12T）。
 * 保证 SCL/SDA 建立/保持时间满足从设备（SSD1306/SH1106/AT24C02/PCF8591）要求，
 * 避免翻转过快导致后半段数据丢失。
 */
#define I2C_DELAY()  do { _nop_(); _nop_(); _nop_(); _nop_(); _nop_(); _nop_(); } while(0)

/* I2C1：OLED 走 P0 口。 */
/* 注：8051 的 P0 为开漏结构，硬件上必须给 SCL/SDA 外接上拉电阻。 */
sbit I2C_SCL = P0^0;
sbit I2C_SDA = P0^1;
/* I2C2：PCF8591 走 P1 口，避免与 OLED 总线抢占。 */
sbit I2C2_SCL = P1^0;
sbit I2C2_SDA = P1^1;
/* I2C3：AT24C02 走 P2.1/P2.0。 */
sbit I2C3_SCL = P2^1;
sbit I2C3_SDA = P2^0;

void I2C_Start(void)
{
    /* 起始条件：SCL 为高时，SDA 从高拉低。 */
    I2C_SDA = 1;
    I2C_SCL = 1;
    I2C_DELAY();
    I2C_SDA = 0;
    I2C_DELAY();
    I2C_SCL = 0;
}

void I2C_Stop(void)
{
    /* 停止条件：SCL 为高时，SDA 从低拉高。 */
    I2C_SDA = 0;
    I2C_DELAY();
    I2C_SCL = 1;
    I2C_DELAY();
    I2C_SDA = 1;
    I2C_DELAY();
}

void I2C_Write(unsigned char dat)
{
    unsigned char i;
    /* 逐位发送 8bit 数据，先发高位。 */
    for (i = 0; i < 8; i++)
    {
        I2C_SDA = (dat & 0x80) ? 1 : 0;
        dat <<= 1;
        I2C_DELAY();
        I2C_SCL = 1;
        I2C_DELAY();
        I2C_SCL = 0;
    }
}

unsigned char I2C_Read(void)
{
    unsigned char i;
    unsigned char dat = 0;
    /* 释放 SDA，由从设备驱动数据线。 */
    I2C_SDA = 1;
    for (i = 0; i < 8; i++)
    {
        dat <<= 1;
        I2C_DELAY();
        I2C_SCL = 1;
        I2C_DELAY();
        if (I2C_SDA)
        {
            dat |= 0x01;
        }
        I2C_SCL = 0;
    }
    return dat;
}

unsigned char I2C_ReadAck(void)
{
    unsigned char ack;
    /* 读应答位：0=ACK，1=NACK。 */
    I2C_SDA = 1;
    I2C_DELAY();
    I2C_SCL = 1;
    I2C_DELAY();
    ack = I2C_SDA;
    I2C_SCL = 0;
    return ack;
}

void I2C_SendAck(unsigned char ack)
{
    /* 主机发送应答位：ack=0 发 ACK，ack=1 发 NACK。 */
    I2C_SDA = ack;
    I2C_DELAY();
    I2C_SCL = 1;
    I2C_DELAY();
    I2C_SCL = 0;
}

void I2C2_Start(void)
{
    I2C2_SDA = 1;
    I2C2_SCL = 1;
    I2C_DELAY();
    I2C2_SDA = 0;
    I2C_DELAY();
    I2C2_SCL = 0;
}

void I2C2_Stop(void)
{
    I2C2_SDA = 0;
    I2C_DELAY();
    I2C2_SCL = 1;
    I2C_DELAY();
    I2C2_SDA = 1;
    I2C_DELAY();
}

void I2C2_Write(unsigned char dat)
{
    unsigned char i;
    for (i = 0; i < 8; i++)
    {
        I2C2_SDA = (dat & 0x80) ? 1 : 0;
        dat <<= 1;
        I2C_DELAY();
        I2C2_SCL = 1;
        I2C_DELAY();
        I2C2_SCL = 0;
    }
}

unsigned char I2C2_Read(void)
{
    unsigned char i;
    unsigned char dat = 0;
    I2C2_SDA = 1;
    for (i = 0; i < 8; i++)
    {
        dat <<= 1;
        I2C_DELAY();
        I2C2_SCL = 1;
        I2C_DELAY();
        if (I2C2_SDA)
        {
            dat |= 0x01;
        }
        I2C2_SCL = 0;
    }
    return dat;
}

unsigned char I2C2_ReadAck(void)
{
    unsigned char ack;
    I2C2_SDA = 1;
    I2C_DELAY();
    I2C2_SCL = 1;
    I2C_DELAY();
    ack = I2C2_SDA;
    I2C2_SCL = 0;
    return ack;
}

void I2C2_SendAck(unsigned char ack)
{
    I2C2_SDA = ack;
    I2C_DELAY();
    I2C2_SCL = 1;
    I2C_DELAY();
    I2C2_SCL = 0;
}

void I2C3_Start(void)
{
    I2C3_SDA = 1;
    I2C3_SCL = 1;
    I2C_DELAY();
    I2C3_SDA = 0;
    I2C_DELAY();
    I2C3_SCL = 0;
}

void I2C3_Stop(void)
{
    I2C3_SDA = 0;
    I2C_DELAY();
    I2C3_SCL = 1;
    I2C_DELAY();
    I2C3_SDA = 1;
    I2C_DELAY();
}

void I2C3_Write(unsigned char dat)
{
    unsigned char i;
    for (i = 0; i < 8; i++)
    {
        I2C3_SDA = (dat & 0x80) ? 1 : 0;
        dat <<= 1;
        I2C_DELAY();
        I2C3_SCL = 1;
        I2C_DELAY();
        I2C3_SCL = 0;
    }
}

unsigned char I2C3_Read(void)
{
    unsigned char i;
    unsigned char dat = 0;
    I2C3_SDA = 1;
    for (i = 0; i < 8; i++)
    {
        dat <<= 1;
        I2C_DELAY();
        I2C3_SCL = 1;
        I2C_DELAY();
        if (I2C3_SDA)
        {
            dat |= 0x01;
        }
        I2C3_SCL = 0;
    }
    return dat;
}

unsigned char I2C3_ReadAck(void)
{
    unsigned char ack;
    I2C3_SDA = 1;
    I2C_DELAY();
    I2C3_SCL = 1;
    I2C_DELAY();
    ack = I2C3_SDA;
    I2C3_SCL = 0;
    return ack;
}

void I2C3_SendAck(unsigned char ack)
{
    I2C3_SDA = ack;
    I2C_DELAY();
    I2C3_SCL = 1;
    I2C_DELAY();
    I2C3_SCL = 0;
}
