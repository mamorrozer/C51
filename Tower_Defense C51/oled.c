...
static unsigned char OLED_WriteDataBlock(const unsigned char code *dat, unsigned char len)
{
    unsigned char i;
    /*
    ...
    I2C_Start();
    I2C_Write(OLED_ADDR_WRITE);
    if (I2C_ReadAck()) { I2C_Stop(); return 0; }
...
static unsigned char OLED_WriteCharAt(unsigned char row, unsigned char col, unsigned char ch)
{
    unsigned char code *glyph;
    unsigned char page;
    unsigned char x;

    if (row >= OLED_TEXT_ROW_MAX || col >= OLED_TEXT_COL_MAX) return 0; /* 越界保护，防止写出显示区。 */

    /* 将不可打印字符映射为 '?'，避免字库越界或显示乱码。
     * 可打印 ASCII 范围：0x20(' ')~0x7E('~')。 */
    if (ch < 0x20 || ch > 0x7E) ch = '?';

    /* 当前字库为 [128][8]，使用 ASCII 直索引。 */
    ...
    OLED_SetPos(page, x);
    return OLED_WriteDataBlock(glyph, OLED_CHAR_WIDTH);
}

unsigned char OLED_WriteCharChecked(unsigned char ch)
{
    if (g_cursor_col >= OLED_TEXT_COL_MAX) return 0; /* 越界保护，防止写入非法列。 */
    if (!OLED_WriteCharAt(g_cursor_row, g_cursor_col, ch)) return 0;
    g_cursor_col++;
    return 1;
}
...
void OLED_WriteChar(unsigned char ch)
{
    ...
    (void)OLED_WriteCharAt(g_cursor_row, g_cursor_col, ch);
    g_cursor_col++;
}
